[CmdletBinding()]
param([switch]$Sample, [switch]$Realtime, [switch]$Build, [ValidateSet('bicycle','stairs','sign','empty','unclear')][string]$Scene = 'bicycle')
...
try {
    ...
    & ./.venv/Scripts/python.exe -m uvicorn backend.app:app --host 127.0.0.1 --port 8000 --ws-max-size 264192 --ws-max-queue 1
