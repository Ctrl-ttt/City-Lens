...
def box_for_distance(label, distance_m, img_w=IMG_W, img_h=IMG_H, y1=100):
    ...
    height_m = {'car': 1.5, 'stairs': 1.2, 'sign': 0.6}[label]
...
def test_all_far_events_leave_empty_list():
    far1 = observe('car', box_for_distance('car', 20.0))
...
def test_degenerate_box_is_kept_because_distance_cannot_be_judged():
    event = observe('car', [100, 200, 300, 200])
...
def test_filter_disabled_when_max_distance_not_positive():
    far = observe('car', box_for_distance('car', 30.0))
...
def test_normalized_box_is_resolution_invariant():
    ...
    box = box_for_distance('car', 4.0)
    event = observe('car', box)
...
def test_invalid_fov_yields_no_estimate_and_keeps_event():
    event = observe('car', [100, 100, 300, 400])
...
def test_http_channel_keeps_far_event_in_details_but_prioritizes_nearer_hazard():
    content = {'events': [
        {'category': 'obstacle', 'label': 'car', 'direction': 'left', 'text': '',
         'box': box_for_distance('car', 30.0)},   # 小图下必然超 5m
    ...
    assert [e['label'] for e in result['events']] == ['stairs', 'car']
