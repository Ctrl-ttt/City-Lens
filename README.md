...
打开 **http://localhost:8000**。勾选样例说明后点击“开始识别”，由使用者允许摄像头访问。样例模式默认返回空场景；“看牌”返回带有“样例牌”前缀的固定文字。点击“测试中文语音”并实际确认声音；没有中文语音时页面显示仅文字状态。当前识别标签不包含自行车。

播报策略可在页面调整：阈值默认为 70 分，只有综合分严格超过阈值才播报；低/中/高详细度分别最多播报 3/2/1 项，并依次增加方位、粗略距离和接近速度等级。速度来自目标框尺度变化，只表示视觉接近趋势，不是米/秒。HTTP 与实时通道均按约 1 fps 抽帧；播报项数不等于抽帧频率。
